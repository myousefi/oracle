...
describe('browser model selection matchers', () => {
  it('includes rich tokens for gpt-5.4', () => {
    const { labelTokens, testIdTokens } = buildModelMatchersLiteralForTest('gpt-5.4');
    expectContains(labelTokens, 'gpt-5.4');
    expectContains(labelTokens, 'gpt-5-4');
    expectContains(labelTokens, 'gpt54');
    expectContains(labelTokens, 'chatgpt 5.4');
    expectContains(testIdTokens, 'gpt-5-4');
    expect(testIdTokens.some((t) => t.includes('gpt-5.4') || t.includes('gpt-5-4') || t.includes('gpt54'))).toBe(true);
  });

  it('includes pro/research tokens for gpt-5.4-pro', () => {
    ...
    expectContains(testIdTokens, 'gpt-5.4-pro');
    expect(testIdTokens.some((t) => t.includes('model-switcher-gpt-5.4-pro'))).toBe(true);
  });
  ...
  it('includes thinking tokens for gpt-5.4-thinking', () => {
    const { labelTokens, testIdTokens } = buildModelMatchersLiteralForTest('gpt-5.4-thinking');
