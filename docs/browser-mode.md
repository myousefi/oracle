...
2. **Automation stack** – code lives in `src/browserMode.ts` and is a lightly refactored version of the `oraclecheap` utility:
   ...
   - Navigates to `chatgpt.com`, switches the model to the requested current GPT-5 target (`GPT-5.4 Pro`, `GPT-5.4 Thinking`, `GPT-5.3 Instant`, or `Auto`), pastes the prompt, waits for completion, and copies the markdown via the built-in “copy turn” button.
...
- `--model`: the same flag used for API runs is accepted, but the ChatGPT automation path resolves GPT aliases onto the current picker targets. Use `gpt-5.4`, `gpt-5.4-thinking`, `gpt-5.3-instant`, or `gpt-5.4-pro`; legacy GPT-5.2/5.1 names are still accepted and normalize to those current targets.
...
- Oracle launches Chrome headful with a persistent automation profile at `~/.oracle/chrome` (override with `ORACLE_BROWSER_PROFILE_DIR` or `browser.manualLoginProfileDir` in `~/.oracle/config.json`).
...
- Chrome stays open by default in manual-login mode (set `browser.keepBrowser=false` in `~/.oracle/config.json` if you want Oracle to close it after runs).
...
- **Model picker drift** – we rely on heuristics to pick the live GPT-5.4/GPT-5.3 targets and still descend into the legacy submenu when needed. If OpenAI changes the DOM we need to refresh the selectors quickly. Consider snapshot tests or a small “self check” command.
